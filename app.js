...
function purchaseCredits(listingId) {
    ...
    if (currentUser.role !== 'company') {
        showNotification('Only companies can purchase carbon credits', 'error');
        return;
    }
    ...
    // For now, show purchase confirmation
    const confirmed = confirm(`Purchase ${listing.quantity} tons of carbon credits for ₹${(listing.price * listing.quantity).toLocaleString('en-IN')}?`);
    
    if (confirmed) {
        showNotification('Purchase request submitted! Our team will contact you to complete the transaction.', 'success');
    }
}
